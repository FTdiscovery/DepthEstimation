...
stereo_im = mpimg.imread(stereo_image_files[0])
# This assumes that all of the pictures have the same dimensions as the previous one.
img_rows, img_cols = stereo_im.shape[0], stereo_im.shape[1] / 2

im_left = stereo_im[:, :int(img_cols)]
im_right = stereo_im[:, int(img_cols):]
...
selectLayer.build([(None, int(img_rows), int(img_cols), 2), (None, int(img_rows), int(img_cols), 3, 30)])

w_net, disp_maps_forward = get_unet(img_rows=int(img_rows), img_cols=int(img_cols))
...
data_generator, _, _, _ = get_data_generators('sample_stereo_images/',
                                                     ...
                                                     batch_size=1,
                                                    shuffle=True, img_rows=int(img_rows), img_cols=int(img_cols))

images = []
depthmaps = []

for i in tqdm(range(len(stereo_image_files))):
    dat = next(data_generator)
    ...
    print(depthMap_left.shape)
    print(depthMap_left)
    print(np.amax(depthMap_left))
    print(np.amin(depthMap_left))


    plt.figure(figsize=(15, 5))
    plt.subplot(1, 2, 2)
    plt.imshow(dat[0][0, :, :int(img_cols), :])
    plt.axis('off')
    plt.subplot(1, 2, 1)
    plt.imshow(depthMap_left, cmap=plt.cm.gist_gray)
    plt.axis('off')
    plt.show()
